...
const { configFilePattern, gherlintrc } = require("./defaults");
const Fs = require("./Fs");
const ruleOptions = require("./rules/_ruleOptions");

module.exports = class GherlintConfig {
    static get DEFAULT_CONFIG_FILENAME() {
        return ".gherlintrc";
    }
    ...
    mergeWithDefaultConfig(userConfig) {
        ...
        keys(userConfig.rules || {}).forEach((rule) => {
            if (typeof userConfig.rules[rule] === "string") {
                mergedRules[rule] = [
                    ...
                    this.defaultConfig.rules[rule][1],
                ];
        ...
        const mergedConfig = merge(this.defaultConfig, userConfig);
    ...
    initializeConfig() {
        ...
        const configPath = join(cwd(), GherlintConfig.DEFAULT_CONFIG_FILENAME);
