...
module.exports = class GherlintConfig {
    ...
    #readConfigFromFile() {
        ...
        if (this.configFile) {
            const extension = extname(basename(this.configFile));
            try {
                if (
                    extension === "" &&
                    extension !== GherlintConfig.CONFIG_EXTENSION.json
                ) {
                    userConfig = JSON.parse(Fs.readFile(this.configFile));
        ...
        if (!isEmpty(userConfig)) {
            ...
            this.config = this.#mergeWithUserConfig(this.config, userConfig);
            this.config = this.#mergeWithCliConfig(
                this.config,
    ...
    #mergeWithUserConfig(defaultConfig, userConfig) {
        const mergedRules = {};
    ...
    #mergeWithCliConfig(config, cliConfig = {}) {
