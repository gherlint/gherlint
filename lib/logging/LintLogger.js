...
const { startTime } = require("../../global/init");
const { formatSeconds, elapsedTime } = require("../../utils/time");
...
module.exports = class LintLogger {
    static log(messages) {
        ...
        keys(messages).forEach((file) => {
            t.underline(file);
            t.dim(` (${formatSeconds(messages[file].elapsedTime)})`);
            t("\n");
        ...
        t.white(` ${pCount} ${problems} (`)
            ...
            .white(")")
            .dim(` (${formatSeconds(elapsedTime(startTime))})\n`);
        return exitCode;
    }
