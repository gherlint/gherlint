...
module.exports = class Indentation extends Rule {
    ...
    execute() {
        if (isEmpty(this._ast?.feature)) return this;
    ...
    fixSingleLine(text, problem) {
        ...
        lines[lineIndex] =
            problem.fixData.replaceBy + lines[lineIndex].trimStart();
    ...
    fixMultiLine(text, problem) {
        ...
        const startLine = lines[startLineIndex].trimStart();
        lines[startLineIndex] = problem.fixData.replaceBy + startLine;
        let endLineIndex = startLineIndex;

        for (const line of lines.slice(startLineIndex + 1)) {
            endLineIndex++;
            lines[endLineIndex] = problem.fixData.replaceBy + line.trimStart();
    ...
    #verify({ keyword, location }) {
        ...
        if (actualIndent !== expectedIndent) {
            let fixProps = {};
            if (this._config.cliOptions.fix) {
                fixProps = {
                    fixData: {
