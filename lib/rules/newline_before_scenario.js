...
module.exports = class NewlineBeforeScenario extends Rule {
    ...
    checkNewlines(astObject) {
        ...
        if (["background", "scenario"].includes(keyword)) {
            ...
            if (keyword === "scenario") {
                ...
                const commentsCount = this.getCommentsCountBetweenLines(
                    this.#lastLine,
                    astObject[keyword].location.line
                );
                lineDiff =
                    lineDiff -
                    this.getTagLines(astObject[keyword].tags) -
                    commentsCount;
    ...
    getTagLines(tags) {
        if (tags.length) {
            const firstTagLine = tags[0].location.line;
            const lastTagLine = tags[tags.length - 1].location.line;
            return lastTagLine - firstTagLine + 1;
        }
        return 0;
    }
    ...
    getCommentsCountBetweenLines(start, end) {
        let commentsCount = 0;
