...
module.exports = class GherlintConfig {
    ...
    triageConfigFiles(configFiles) {
        const orderedConfigFiles = [];
        for (const configFileName of configFileOrder) {
            const matchedFile = configFiles.find((file) =>
                file.endsWith(configFileName)
            );
            matchedFile && orderedConfigFiles.push(matchedFile);
        }
        return orderedConfigFiles;
    }
    ...
    searchConfigFile() {
        ...
        if (matches.length > 1) {
            const configFiles = this.triageConfigFiles(matches);
            log.info("Found multiple config files:", configFiles.join("\n  "));
            log.info(`Using config file '${basename(configFiles[0])}'`);
